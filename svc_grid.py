...
#Set grid parameters
C_VAL = [0.05,0.1,0.15,0.2,0.25,0.3, 0.35, 0.4, 0.45, 0.5, 0.55, 0.6, 0.65, 0.7, 0.75, 0.8, 0.85, 0.9, 0.95, 1.0, 1.05, 1.1]
param_grid = {
    'C': C_VAL,
}

#Fit the data for each grid point
clf = svm.LinearSVC(loss="l2",penalty="l2",dual=False)
grid = GridSearchCV(clf, param_grid=param_grid, n_jobs=3, cv=4, verbose=10)
...
print(array)
np.savetxt('crossVal_linearSVC_Results_l2.csv', array,delimiter=',')

GridCV_scores_printer.print_GridCV_scores(grid,"linearSVC_results_l2.csv")

#Print out a prediction if needed
